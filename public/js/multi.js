...
function getEls(ids) {
  ...
  return {
    ...
    theadEl: pick('thead'),
    tbodyEl: pick('tbody'),
    searchEl: pick('search'),
    filtersPanelEl: pick('filtersPanel'),
    filtersClearBtn: pick('filtersClearBtn'),
    pageSizeEl: pick('pageSize'),
    prevBtn: pick('prevBtn'),
    nextBtn: pick('nextBtn'),
    pageInfoEl: pick('pageInfo'),
    selectEl: pick('select'),
  };
}

export function createView(ids, options = {}) {
  const els = getEls(ids);
  const state = createState();
  const onAfterLoad = options.onAfterLoad || (() => {});
  ...
  function applyFilter() {
    ...
    const st = state.filtersByTable[state.currentTable] || {};
    let filtered = rows.filter((row) => {
      for (const [col, cfg] of Object.entries(st)) {
        const val = row?.[col];
        if (cfg.type === 'eq') {
  ...
  async function loadAndRender(table) {
    ...
    try {
      ...
      onAfterLoad(getVisibleColumns(state.currentTable, columnsByTable));
    } catch (err) {
      console.error(err);
  ...
  return {
    ...
    getFilteredRows: () => state.currentFiltered.slice(),
    getVisibleColumns: () => getVisibleColumns(state.currentTable, columnsByTable),
  };
}